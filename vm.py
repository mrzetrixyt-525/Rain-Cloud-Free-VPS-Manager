...
SSHX_TOTAL_TIMEOUT = env_int("SSHX_TOTAL_TIMEOUT", 90, 20, 240)
...
chmod 700 "$BASE" "$STATE_DIR" 2>/dev/null || true

printf '%s\n' "[⚡] RGNODES™ • SSHx" "[•] Session: $NAME" "[•] Base: $BASE"

# SSHx official installer prerequisite.
...
# Use the exact official installer requested by RGNODES.
if [ ! -x "$BIN" ]; then
    printf '%s\n' "[•] Downloading official SSHx binary..."
    if ! curl -sSf https://sshx.io/get | sh -s download >"$BASE/install.log" 2>&1; then
        printf '%s\n' "[❌] SSHx download failed."
        tail -n 40 "$BASE/install.log" 2>/dev/null || true
        exit 12
    fi
fi

# Resolve installer output. The official installer can place sshx in the
# current directory or a user PATH depending on platform/installer version.
if [ ! -x "$BIN" ] && command -v sshx >/dev/null 2>&1; then
    RESOLVED_BIN="$(command -v sshx)"
    if [ "$RESOLVED_BIN" != "$BIN" ]; then
        cp -f "$RESOLVED_BIN" "$BIN" 2>/dev/null || true
    fi
fi

if [ ! -x "$BIN" ]; then
    printf '%s\n' "[❌] SSHx binary was not installed correctly."
    tail -n 40 "$BASE/install.log" 2>/dev/null || true
    exit 13
fi
chmod 700 "$BIN" 2>/dev/null || true

# Reuse the existing process when possible. This avoids killing a healthy
# session merely because the URL is not immediately visible.
existing_pid=""
if [ -s "$PID" ]; then
    existing_pid="$(cat "$PID" 2>/dev/null || true)"
fi
case "$existing_pid" in ''|*[!0-9]*) existing_pid="";; esac

if [ -n "$existing_pid" ] && kill -0 "$existing_pid" 2>/dev/null; then
    if [ -s "$URL" ]; then
        printf '%s\n' "[✅] Existing SSHx session is already online."
        cat "$URL"
    else
        printf '%s\n' "[✅] Existing SSHx process is online; URL collector remains active."
    fi
    cp -f "$PID" "$STATE_DIR/sshx.pid" 2>/dev/null || true
    [ -s "$URL" ] && cp -f "$URL" "$STATE_DIR/sshx.url" 2>/dev/null || true
    cp -f "$LOG" "$STATE_DIR/sshx.log" 2>/dev/null || true
    exit 0
fi
...
# Start the SSHx session immediately.
nohup "$BIN" --quiet --name "$NAME" >"$LOG" 2>&1 </dev/null &
SSHX_PID=$!
printf '%s\n' "$SSHX_PID" > "$PID"

printf '%s\n' "[✅] SSHx STARTED" "[•] PID: $SSHX_PID" "[•] LOG: $LOG" "[•] NAME: $NAME" | tee -a "$LOG"

# The sshx CLI can initialize asynchronously. Do NOT block the bot waiting
# for the browser URL. A tiny detached collector watches the process output
# and persists the complete encrypted link once sshx emits it.
(
    set +e
    for _ in $(seq 1 120); do
        if [ -s "$LOG" ]; then
            CLEAN="$(sed -E 's/\x1B\[[0-9;?]*[ -\/]*[@-~]//g' "$LOG" 2>/dev/null | tr '\r' '\n')"
            URL_NOW="$(printf '%s\n' "$CLEAN" | grep -Eo 'https://sshx\.io/s/[A-Za-z0-9_-]+#[^[:space:]<>\[\]"'"'"'`\\[\\]]+' | tail -n 1)"
            if [ -n "$URL_NOW" ]; then
                printf '%s\n' "$URL_NOW" > "$URL"
                chmod 600 "$URL" 2>/dev/null || true
                cp -f "$URL" "$STATE_DIR/sshx.url" 2>/dev/null || true
                cp -f "$LOG" "$STATE_DIR/sshx.log" 2>/dev/null || true
                exit 0
            fi
        fi
...
printf '%s\n' "$SSHX_PID" > "$STATE_DIR/sshx.pid" 2>/dev/null || true
cp -f "$LOG" "$STATE_DIR/sshx.log" 2>/dev/null || true

if kill -0 "$SSHX_PID" 2>/dev/null; then
    printf '%s\n' "[✅] SSHx process ONLINE • PID $SSHX_PID"
    # Return success even when the URL has not appeared yet. The process and
    # collector are now running, and the next Console request can read it.
    [ -s "$URL" ] && cat "$URL"
    exit 0
fi
...
def normalize_sshx_url(raw: str | None) -> str | None:
    """Validate and return a complete SSHx browser URL, including its E2E key."""
    text = str(raw or "").replace("\r", " ").replace("\n", " ")
    # Deliberately locate the URL without requiring a particular key alphabet.
    # sshx's encryption key is carried in the URL fragment and must never be
    # discarded, normalized, or re-encoded by the bot.
    match = re.search(r"https://sshx\.io/s/[A-Za-z0-9_-]+#[^\s<>\[\]\"']+", text, flags=re.I)
    if not match:
        return None
    ...
    if not re.fullmatch(r"/s/[A-Za-z0-9_-]+", parsed.path):
        return None
    # A share URL without a fragment is exactly the broken URL shown by the
    # browser as "invalid end-to-end encryption key".
...
async def install_and_start_sshx(container: str) -> dict[str, str] | None:
    """Start/reuse SSHx without making URL emission block VPS operations.

    SSHx can need a short asynchronous initialization before it prints the
    browser URL. The container-side launcher therefore starts SSHx and a
    detached collector immediately; this function first checks durable state,
    then launches once, and finally performs only a very short state read.
    """
    if await docker_state(container) != "running":
    ...
    if saved_pid and await sshx_process_alive(container, saved_pid):
        if saved_url:
            logger.info("Reusing existing SSHx session for %s.", clean(container, 32))
            return {"url": saved_url, "pid": saved_pid}
        logger.info("SSHx process already running for %s; URL collector still initializing.", clean(container, 32))
        return {"url": "", "pid": saved_pid}

    try:
        rc, out, err = await docker_exec_shell(
            container,
            SSHX_INSTALL_SCRIPT,
            timeout=45,
        )
    except asyncio.TimeoutError:
        logger.warning("SSHx launcher safety timeout for %s; checking durable state.", clean(container, 32))
        saved_url, saved_pid = await _read_sshx_state(container)
        if saved_pid and await sshx_process_alive(container, saved_pid):
            return {"url": saved_url or "", "pid": saved_pid}
        return None
    except Exception as exc:
        logger.warning("SSHx launcher failed for %s: %s", clean(container, 32), safe_log(exc))
        return None
    ...
    saved_url, saved_pid = await _read_sshx_state(container)
    final_url = saved_url or url
    if saved_pid and await sshx_process_alive(container, saved_pid):
        if final_url:
            logger.info("SSHx ready for %s (PID %s).", clean(container, 32), saved_pid)
        else:
            logger.info("SSHx started for %s (PID %s); encrypted URL is still initializing.", clean(container, 32), saved_pid)
        return {"url": final_url or "", "pid": saved_pid}
...
async def docker_reinstall_vps(vps: sqlite3.Row, os_type: str) -> tuple[bool, str]:
    ...
    try:
        ...
        if old_exists:
            with contextlib.suppress(Exception):
                await docker_remove(old_container)
...
async def create_console_access(vps: sqlite3.Row, user: discord.User | discord.Member) -> tuple[bool, str]:
    ...
    if not console:
        ...
        return False, "SSHx failed to start. The VPS is still online. Check the VPS SSHx log and press Console again."
