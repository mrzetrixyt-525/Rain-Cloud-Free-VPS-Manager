...
def choose_firewall():
    ...
    if forced in {"nftables", "iptables", "windows"}:
        if forced == "windows" and not windows():
            return "unsupported"
        if forced in {"nftables", "iptables"} and not linux():
            return "unsupported"
        return forced
...
def init_firewall():
    backend = choose_firewall()
    state["firewall"]["backend"] = backend
    ...
    if backend == "nftables":
        ...
        ok, reason = False, "no supported firewall backend detected"

    state["firewall"]["ready"] = ok
    state["firewall"]["reason"] = reason
    return ok
...
def dashboard_token_ok(path, headers):
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        path = parsed.path
        if not dashboard_token_ok(self.path, self.headers):
            self.send_body(
                401,
                b"Unauthorized",
